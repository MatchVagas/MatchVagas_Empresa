...
public class CadastroVagaFragment extends Fragment {
    ...
    private CadastroVagaViewModel viewModel;
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        viewModel = new ViewModelProvider(this).get(CadastroVagaViewModel.class);
        ...
        viewModel.getPublicarResult().observe(getViewLifecycleOwner(), resource -> {
            hideLoading();
            view.findViewById(R.id.btn_publicar).setEnabled(true);
            switch (resource.getStatus()) {
                case SUCCESS:
                    Toast.makeText(requireContext(), "Vaga publicada!", Toast.LENGTH_SHORT).show();
                    Navigation.findNavController(view.findViewById(R.id.btn_publicar)).navigateUp();
                    break;
                case ERROR:
    ...
    private void publicarVaga(View view, View btn) {
        ...
        request.empresaId    = session.getEmpresaId();
        request.titulo       = getText(etTitulo);
        request.descricao    = getText(etDesc);
        request.requisitos   = getText(etReq);
        request.tipoVagaId   = getSelectedId(repo.getTiposVaga(),    actvTipo.getText().toString());
        request.modalidadeId = getSelectedId(repo.getModalidades(),  actvModalidade.getText().toString());
        request.escolaridadeId = getSelectedId(repo.getEscolaridades(), actvEscolaridade.getText().toString());
        request.cidadeId     = getSelectedId(repo.getCidades(),      actvCidade.getText().toString());
        List<LookupItem> statusVaga = repo.getStatusVaga();
        request.statusVagaId = statusVaga.isEmpty() ? null : statusVaga.get(0).id;
        request.areaAtuacao  = getText(etArea);
        ...
        viewModel.publicarVaga(request);
    }
    ...
    private boolean validarFormulario() {
        ...
        if (getSelectedId(repo.getTiposVaga(), actvTipo.getText().toString()) == null) { tilTipo.setError(getString(R.string.error_selecione_opcao)); if (primeiroErro == null) primeiroErro = tilTipo; valido = false; }
        if (getSelectedId(repo.getModalidades(), actvModalidade.getText().toString()) == null) { tilModalidade.setError(getString(R.string.error_selecione_opcao)); if (primeiroErro == null) primeiroErro = tilModalidade; valido = false; }
        ...
        if (getSelectedId(repo.getEscolaridades(), actvEscolaridade.getText().toString()) == null) { tilEscolaridade.setError(getString(R.string.error_selecione_opcao)); if (primeiroErro == null) primeiroErro = tilEscolaridade; valido = false; }
        if (getSelectedId(repo.getCidades(), actvCidade.getText().toString()) == null) { tilCidade.setError(getString(R.string.error_selecione_opcao)); if (primeiroErro == null) primeiroErro = tilCidade; valido = false; }
